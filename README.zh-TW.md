...
- **【新功能】開播桌面通知**：追蹤或最愛的成員開播時接收即時且簡潔的桌面通知，包含直播標題，點擊即可直接在新分頁中開啟並前往觀看（支援多語言顯示名稱、通知範圍篩選功能）。
- **【新功能】自動開啟最愛頻道**：點擊彈出式視窗側邊欄底部的閃電按鈕，即可在最愛的 VTuber 開播時自動開啟新分頁（支援前景/背景分頁設定，以及下播後自動重新啟用/跳轉的實驗性功能，並可設定最大連續播放流數量）。
...
- **【新功能】匯入/匯出設定**：可將目前的設定、最愛與追蹤頻道、自訂頻道以及側邊欄配置匯出為 JSON 備份檔案（基於安全性考量，會自動除外 API 金鑰），並支援在其他裝置上進行匯入以快速還原設定。
